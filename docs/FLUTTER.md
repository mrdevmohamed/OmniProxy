...
**`MockApiClient`** (`app/lib/core/mock_api_client.dart:17`) is not a test double that fakes happy paths — it is a *faithful in-memory implementation of the contract*: same method names, same response shapes, same error codes (`not_found`, `connected`, `busy`, `validation_failed`), and it *emits the same async events* (`stateChanged`, `logAppended`, `latencyTested`) with simulated delays (`connectDelay` 600ms, `latencyDelay` 250ms). It even seeds three servers (VLESS/Tokyo, Shadowsocks/Frankfurt, HTTP/localhost — `:41-83`) and implements filter/sort/favorite/import/export semantics. Its stated purpose (doc comment `:9-16`) is wiring the M5 shell *before* the real transports landed; it remains the Windows fallback (see §10). It has a `dispose()` that closes the controller (`:489`) — the one method not on the interface.
...
- Content slot below (`:34-51`) is state-driven: `error` → `_ErrorBanner` showing `session.error.message`; connected/connecting with a known server → `_ServerCard`; otherwise `_PlaceholderCard` ("No servers yet. Add a server to get started." with an "Add server" button that jumps to the Servers tab via `onNavigateToServers`, or "Choose a server to connect.").
...
- Target selection (`:67-85`) — the session's `serverId` if one exists, else the favorite server, else the first; `_connect` re-reads providers rather than trusting build-scope values (guard against stale closes).
...
- `_ServerCard` (`:481`) — protocol abbreviation tile, name (+favorite star), `protocol · address:port` (+ ` · ws` transport label), latency pill ("—" if untested), and a popup menu: Test latency / Mark favorite / Disable / Duplicate / Export / Delete. Disabled servers render at 55% opacity (`:507`). Delete needs a confirm dialog (`:337`).
...
`LogsNotifier` buffer rendered with `ListView(reverse: true)` (`:95-104`) so the newest entry stays pinned to the bottom — the standard live-terminal pattern. Level filter is a `DropdownButton<LogLevel?>` (null = all) filtering by a `rank` extension (`:198-221`); color coding per level. Toolbar has Refresh (`notifier.refresh`, re-seeds) and Clear (`notifier.clear`). Empty state explains how to produce output ("Connect to a server or adjust the filter"). Entries render local-time HH:MM:SS, level label, component (primary-colored), and message in monospace (`:120-160`). The doc comment (`:7-8`) restates the security invariant: entries come from core's *redacted* stream and never contain credentials.
...
**Windows (`bridge_windows.dart`)** — **stub.** `WindowsBridge` implements `BridgeTransport` but `request()` and `events` throw `UnsupportedError('WindowsBridge lands in M8')` (`:10,15`); `start`/`stop` are no-ops (`:18-21`). Critically, **`client_factory.dart` never constructs `WindowsBridge`** — its fallback for non-Android/non-Linux is `MockApiClient()` (`:27`). So on Windows today the app boots against the mock, and the `WindowsBridge` class exists only to (a) document the intended ABI and (b) be swapped in when the DLL + Wintun path is verified (M8 is "code-complete, documented untested on the Linux host", `docs/platform-notes.md:80`). The doc comment on the file (`:6`) explicitly says it's "documented untested on the Linux host."
