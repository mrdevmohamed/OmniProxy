...
- `make apk` / `appbundle` run `flutter build apk|appbundle --release` after rebuilding the AAR (`Makefile:104-107`). The AAR must be rebuilt on every core change — it is checked in as a build artifact, not built by Gradle. Release APKs are built **per-ABI** (`flutter build apk --release --split-per-abi`, `Makefile:128-132`): three small APKs — `armeabi-v7a`, `arm64-v8a`, `x86_64` — each carrying only its own `libgojni.so`, instead of one fat APK with all four (the AAR's x86 ABI is only exercised by `make apk-fat`'s universal build). `make stage-android` copies them into `dist/omniproxy-android-<abi>.apk`, and `.github/workflows/android.yml` uploads all of them as artifacts / Release assets.
