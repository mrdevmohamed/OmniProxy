...
- **Service model:** a foreground `VpnProxyService` hosts the Go core in both connection modes and shows a persistent, non-dismissible notification while connected (PRD §3.1).
  ...
  - **Connect/disconnect serialization:** `connect` and `disconnect` run on a single lifecycle executor in the native bridge (`Bridge.submitConnect`/`Bridge.submitDisconnect`), so a disconnect can never race an in-flight connect. A disconnect bumps a generation counter; a queued connect that predates it aborts instead of starting the engine. The foreground host is torn down **after** the core confirms the disconnect, and the VpnService owns the TUN `ParcelFileDescriptor` and closes it in `onDestroy` (a disconnect closes it only once, after the engine has released its duplicated fd). Both services return `START_NOT_STICKY` — they are explicitly started/stopped hosts and must never be resurrected by the system.
...
- **Go core packaging:** `go build -buildmode=c-shared -tags with_gvisor` → `omniproxy.dll`, loaded via `dart:ffi`; `wintun.dll` bundled next to the binary for TUN (sing-box/Wintun). Same `GOMOD_TAGS` as the Android AAR and Linux helper, so the gVisor user-space stack is available for Windows VPN mode.
- **TUN:** Wintun driver (gVisor stack available via the `with_gvisor` tag). Elevated privileges for interface creation are handled per sing-box's Windows model; the FFI process is the app process.
...
- **Build:** core DLL cross-compiles from Linux (`make windows-core`) with the `with_gvisor` tag; the Flutter bundle requires a Windows host or CI — `.github/workflows/windows.yml` builds both on `windows-latest`. The glue is cross-checked by `make go-check` (`go-check-windows`, same tag).
