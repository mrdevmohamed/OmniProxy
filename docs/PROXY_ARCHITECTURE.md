...
- **`allowInsecure`** disables certificate validation (`tlsContainer` sets `Insecure`, `engine/config.go:427`). The model comment and contract require it to *never* be on in the default flow: "Certificate validation is on by default; bypass only in Advanced Mode with an explicit user-visible warning" (`core/models/server.go:59-61`; `docs/api-contract.md:39-40`). **Implemented:** the toggle on the TLS section is locked unless Advanced Mode is enabled (Settings → Advanced), and enabling it shows a "Disable certificate validation?" confirmation dialog (`app/lib/features/servers/server_edit_screen.dart:333-354,458-466`; `app/lib/features/settings/settings_screen.dart:129-158`; `app/lib/core/models.dart:591`).
...
- **What Advanced Mode gates:** `AppSettings.advancedModeEnabled` (`app/lib/core/models.dart:591`) is now toggled from Settings → Advanced (`app/lib/features/settings/settings_screen.dart:129-158`) and currently enforces one thing: the cert-bypass toggle on the server form is locked unless enabled, with an explicit confirmation dialog on enable (`server_edit_screen.dart:333-354`). Its Phase 2 purpose remains gating the routing/tunnel builders (`core/models/server.go:59-61`; `docs/implementation-plan.md:19`).
...
| **Advanced Mode (cert bypass gating, advanced settings)** | `AppSettings.AdvancedModeEnabled` stored (`core/models/settings.go:86`); allow-insecure flag exists in the model and builder (`engine/config.go:427`) | ~~UI gating + warning dialog~~ **DONE** — locked toggle + confirmation dialog (`server_edit_screen.dart:333-354,458-466`) |
...
7. ~~**`allowInsecure` is present but Advanced-Mode-gated only in Phase 2**~~ — **RESOLVED**: the toggle is locked outside Advanced Mode and enabling it requires an explicit confirmation dialog (`server_edit_screen.dart:333-354,458-466`; `settings_screen.dart:129-158`).
